    ...
    callback: async ({ message, args, interaction, client, guild, channel, instance, member: staff, options, prefix, text }) => {
        if (message.member?.roles.cache.has('854031216487432214') || message.author.id === '946355425631141901') {      
            ...
            if (userId === '946355425631141901') {
                return `You Can't Veteran <@${946355425631141901}> He's My **Dev**`
            }
            if (userId === '929295453487501372') {
                return `You Can't Veteran <@${929295453487501372}> He's **The Owner**`
            }
            if (userId === '590945703661010985') {
                return `You Can't Veteran <@${590945703661010985}> He's **The Owner**`
            }
            if (userId === '363032092231073795') {
                return `You Can't Veteran <@${363032092231073795}> He's **The Owner**` 
            }
            if (userId === '394508186826047499') {
                return `You Can't Veteran <@${394508186826047499}> He's **The Manager**`
            }
            if (userId === message.author.id) {
                return `You can't Veteran **Your Self** `
            }
            ...
            if (result) {
                return `<@${userId}> Is Already Veteran`
            } 
            ...
            try {
                ...
                return `Cannot Make a Veteran That User`
            }
            return `Made Veteran <@${userId}> For "${Duration}"`
        }
    },
} as ICommand