    ...
    callback: async ({ message, args, interaction, client, guild, channel, instance, member: staff, options, prefix, text }) => {
        if (message.member?.roles.cache.has('854031216487432214') || message.author.id === '946355425631141901') {      
            ...
            if (userId === '946355425631141901') {
                return `You Can't Jigsaw <@${946355425631141901}> He's My **Dev**`
            }
            if (userId === '929295453487501372') {
                return `You Can't Jigsaw <@${929295453487501372}> He's **The Owner**`
            }
            if (userId === '590945703661010985') {
                return `You Can't Jigsaw <@${590945703661010985}> He's **The Owner**`
            }
            if (userId === '363032092231073795') {
                return `You Can't Jigsaw <@${363032092231073795}> He's **The Owner**` 
            }
            if (userId === '394508186826047499') {
                return `You Can't Jigsaw <@${394508186826047499}> He's **The Manager**`
            }
            if (userId === message.author.id) {
                return `You can't Jigsaw **Your Self** `
            }
            ...
            if (result) {
                return `<@${userId}> Is Already Jigsaw`
            } 
            ...
            try {
                ...
                if (member) {
                    member.roles.add('799234140468674561')
                }
            } catch (ignored) {
                return `Cannot Make a Jigsaw That User`
            }
            return `Made Jigsaw <@${userId}> For "${Duration}"`
        }
    },
} as ICommand