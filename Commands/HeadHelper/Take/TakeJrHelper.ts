...
export default {
    ...
    callback: ({ message, args, interaction, client, guild }) => {
        if (message.member?.roles.cache.has('854031216487432214') || message.author.id === '880894680672395294' || message.member?.permissions.has('ADMINISTRATOR')) {
            // Member ID
            ...
            if (MEMBER_ID === message.author.id) {
                ...
                return `Took JrHelper From <@${MEMBER_ID}> `
            }
